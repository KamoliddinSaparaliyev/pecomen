const elBox = document.querySelector(".box-card");
const elSearchForm = document.querySelector("#search-form");
const elSearch = document.querySelector("#search");
const elForm = document.querySelector("#form");

const renderPacemon = (array, parent = elBox) => {
  parent.textContent = " ";
  for (let i = 0; i < array.length; i++) {
    const element = array[i];

    const card = document.createElement("div");
    card.className = "card border border-dark rounded-5";
    ...
    card.innerHTML = `
                ...
                <div class="card-body d-flex gap-4 pb-4 ">
                    <h5 class="card-title ">${element.weight}  </h5>
                    <h5 class="card-title ">${element.height} </h5>
                </div>
...
renderPacemon(pokemons);

elSearchForm.addEventListener("input", (e) => {
  e.preventDefault();

  const searchContent = elSearch.value.toLowerCase();
  const searchResults = [];

  pokemons.forEach((film) => {
    if (film.name.toLowerCase().trim().includes(searchContent)) {
      searchResults.push(film);
    }
  });

  renderPacemon(searchResults);
});
